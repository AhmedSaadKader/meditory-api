...
export class DrugsService {
   ...
   * Allows filtering by drug name AND/OR ingredient simultaneously
   *
   * @param drugName - Drug name filter (supports * and ? wildcards)
   * @param ingredient - Ingredient filter (supports * and ? wildcards)
   * @param dosageForm - Optional: filter by dosage form
   * @param route - Optional: filter by route
   * @param priceMin - Optional: minimum price
   * @param priceMax - Optional: maximum price
  ...
  async searchAdvanced(
    ...
    const sanitizedDrugName = drugName ? sanitizeSearchTerm(drugName) : null;
    const sanitizedIngredient = ingredient
      ? sanitizeSearchTerm(ingredient)
      : null;

    // Build SQL query using advanced search function
    let sql = `
      SELECT DISTINCT
        dis.drug_name,
        dis.company,
