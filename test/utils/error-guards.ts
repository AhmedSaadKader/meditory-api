...
export const loginGuard = createErrorGuard<{ success: boolean; token: string; user: any }>(
  (input) => input?.success === true && input?.token && input?.user
);

export const registerGuard = createErrorGuard<{ success: boolean; message: string; user: any }>(
